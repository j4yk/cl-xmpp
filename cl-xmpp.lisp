...
(defmethod xml-element-to-event ((connection connection)
  ...
  (make-instance 'message
		 ...
		 :to (value (get-attribute object :to))
                 ...
                 :type (value (get-attribute object :type))
		 :body (if (get-element object :body)
			   (data (get-element (get-element object :body) :\#text))
			   "nil")))

(defmethod stanza-waiting-p ((connection connection))
  "listens into the socket stream and checks whether stanzas have arrived"
  (listen (usocket:socket-stream (server-socket connection))))
